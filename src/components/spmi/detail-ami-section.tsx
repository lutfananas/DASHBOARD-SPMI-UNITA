...
export function DetailAMISection() {
  const totalIndikator = DOKUMEN_AMI_DETAIL.reduce(
    (sum, doc) => sum + doc.indikator.length, 0
  );
  const totalTercapai = DOKUMEN_AMI_DETAIL.reduce(
    (sum, doc) => sum + doc.indikator.filter(i => i.status === "Tercapai").length, 0
  );
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <ArkhamCard variant="primary" withTerminalBorder>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-base font-semibold text-sky-300 font-mono uppercase tracking-wide">
              Detail Hasil AMI per Dokumen
            </h3>
            <p className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider">
              13 Dokumen AMI 2024-2025 • {totalIndikator} Indikator • {totalTercapai} Tercapai ({persenTercapai}%) • {totalBelum} Belum Tercapai
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div className="p-3 rounded border border-sky-500/15 bg-sky-500/5">
            ...
            <p className="text-2xl font-bold tabular-nums font-mono text-sky-300">{DOKUMEN_AMI_DETAIL.length}</p>
          </div>
      ...
      <Accordion type="single" collapsible className="space-y-3">
        {DOKUMEN_AMI_DETAIL.map((doc, idx) => {
          const tercapai = doc.indikator.filter(i => i.status === "Tercapai").length;
          const belum = doc.indikator.filter(i => i.status === "Belum Tercapai").length;
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {doc.indikator.map((ind) => (
                        <TableRow
                          key={ind.no}
                          className={`border-sky-500/8 ${ind.status === "Belum Tercapai" ? "bg-amber-500/[0.03]" : ""}`}
                        >
                          <TableCell className="font-mono text-xs text-muted-foreground tabular-nums">
                            {String(ind.no).padStart(2, "0")}
                          </TableCell>
                          <TableCell className="text-sm">
                            <span className={ind.status === "Belum Tercapai" ? "text-amber-300/90" : "text-foreground"}>
                              {ind.indikator}
                            </span>
                          </TableCell>
                          <TableCell>
                            <span className="text-xs font-mono font-bold tabular-nums text-sky-300">
                              {ind.capaian}
                            </span>
                          </TableCell>
                          <TableCell>
